...
class ActivitiesScreen extends StatefulWidget {
  ...
  const ActivitiesScreen({Key? key, required this.id}) : super(key: key);
...
class _ActivitiesScreenState extends State<ActivitiesScreen>
  ...
  final List<Tab> tabs = <Tab>[
    ...
    const Tab(text: "Add Activities Detail(s)"),
  ];
  ...
  Future getData() async {
    ...
    try {
      ...
      var headers = {"Authorization": "Bearer ${tokens!}"};
      var url =
          Uri.parse('$baseUrlTest/api/v1/safari_activities/index/${widget.id}');
  ...
  Future deleteData(id) async {
    ...
    try {
      var tokens = await SharedPreferences.getInstance()
          .then((prefs) => prefs.getString('token'));
      var headers = {"Authorization": "Bearer ${tokens!}"};
      var url = Uri.parse('$baseUrlTest/api/v1/safari_activities/destroy/$id');
  ...
  Future UpdateTasks(id) async {
    try {
      ...
      var tokens = await SharedPreferences.getInstance()
          .then((prefs) => prefs.getString('token'));
      var headers = {"Authorization": "Bearer ${tokens!}"};
      var url = Uri.parse('$baseUrlTest/api/v1/safari_activities/update');
      print(url);
      ...
      body: TabBarView(
        ...
        children: tabs.map((Tab tab) {
          return SingleChildScrollView(
              child: tab.text == "Activities" ? listMember() : membersForm());
        }).toList(),
      ),
    );
  }
  ...
  Future createTourMember() async {
    try {
      var tokens = await SharedPreferences.getInstance()
          .then((prefs) => prefs.getString('token'));
      var headers = {"Authorization": "Bearer ${tokens!}"};
      var url = Uri.parse('$baseUrlTest/api/v1/safari_activities/store');
      //print(tentcapVal);
