...
class FilmingScreen extends StatefulWidget {
  final String id;
  const FilmingScreen({Key? key, required this.id}) : super(key: key);
...
class _FilmingScreenState extends State<FilmingScreen>
  ...
  final List<Tab> tabs = <Tab>[
    const Tab(text: "Filming"),
    const Tab(text: "Add Filming Detail(s)"),
  ];
  ...
  List Data = [];
  List data1 = [];
  Future getFilming() async {
    try {
      var tokens = await SharedPreferences.getInstance()
          .then((prefs) => prefs.getString('token'));
      var headers = {"Authorization": "Bearer ${tokens!}"};
      var url = Uri.parse('$baseUrlTest/api/v1/tourism_activity_type');
  ...
  Future getData() async {
    ...
    try {
      var tokens = await SharedPreferences.getInstance()
          .then((prefs) => prefs.getString('token'));
      var headers = {"Authorization": "Bearer ${tokens!}"};
      var url = Uri.parse('$baseUrlTest/api/v1/safari_film/index/${widget.id}');
  ...
  Future deleteData(id) async {
    try {
      var tokens = await SharedPreferences.getInstance()
          .then((prefs) => prefs.getString('token'));
      var headers = {"Authorization": "Bearer ${tokens!}"};
      var url = Uri.parse('$baseUrlTest/api/v1/safari_film/destroy/$id');
              ...
              SafeArea(
                child: Padding(
                  ...
                  child: DropdownButtonFormField<String>(
                    ...
                    iconEnabledColor: Colors.black,
                    items:
                        catType.map<DropdownMenuItem<String>>((String value) {
                      return DropdownMenuItem(
                        value: value,
                        child: Container(
                          width: double.infinity,
  ...
  Future UpdateTasks(id) async {
    try {
      ...
      var tokens = await SharedPreferences.getInstance()
          .then((prefs) => prefs.getString('token'));
      var headers = {"Authorization": "Bearer ${tokens!}"};
      var url = Uri.parse('$baseUrlTest/api/v1/safari_film/update');
      ////print(url);
      ////print(visitorTypes);
  ...
  message(String hint, String message, {int? index, bool? isdelMessage}) {
    return Alert(
      context: context,
      ...
      body: TabBarView(
        ...
        children: tabs.map((Tab tab) {
          return SingleChildScrollView(
              child: tab.text == "Filming" ? listMember() : membersForm());
        }).toList(),
      ),
    );
  }
                                  ...
                                  child: Row(children: [
                                    ...
                                    Expanded(
                                      ...
                                      child: Column(
                                          ...
                                          children: [
                                            Text(
                                                "Duration Type: ${Data[index]["duration_type"]}",
                                                style: const TextStyle(
                                                    color: Colors.black,
                                                    fontSize: 15,
                                                    fontWeight:
  ...
  Future createTourMember() async {
    try {
      var tokens = await SharedPreferences.getInstance()
          .then((prefs) => prefs.getString('token'));
      var headers = {"Authorization": "Bearer ${tokens!}"};
      var url = Uri.parse('$baseUrlTest/api/v1/safari_film/store');
      //////print(tentcapVal);
